...
namespace Sharphound.Runtime
{
    public class OutputWriter
    {
        ...
        private string ZipFiles()
        {
            ...
            {
                _computerOutput.GetFilename(),
                _userOutput.GetFilename(),
                _groupOutput.GetFilename(),
                _containerOutput.GetFilename(),
                _domainOutput.GetFilename(),
                _gpoOutput.GetFilename(),
                _ouOutput.GetFilename(),
                _rootCAOutput.GetFilename(),
                _aIACAOutput.GetFilename(),
                _enterpriseCAOutput.GetFilename(),
                _nTAuthStoreOutput.GetFilename(),
                _certTemplateOutput.GetFilename(),
                _issuancePolicyOutput.GetFilename()
            });

            foreach (var entry in fileList.Where(x => !string.IsNullOrEmpty(x)))
            {
                var fi = new FileInfo(entry);
                // Modify the filename before creating the zip entry
                // Change the word <RandomWord> with your word
                var newFileName = Path.GetFileNameWithoutExtension(fi.Name) + "RandomWord" + fi.Extension;
                var zipEntry = new ZipEntry(newFileName) { DateTime = fi.LastWriteTime, Size = fi.Length };
